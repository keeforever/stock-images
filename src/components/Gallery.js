  ...
  return (
    <>
      <section className="gallery-container">
        {!mounted.current
          ? !(isLoading.boo || isError.boo)
            ? oldData.map((image, index) => <Photo key={index} image={image} />)
            : null
          : oldData.map((image, index) => <Photo key={index} image={image} />)}
      </section>
      ...
      {isLoading.boo ? (
        ...
        <h2 className="info-center">{isError.status}</h2>
      ) : apiData.results !== undefined ? (
        apiData.results.length < 1 ? (
          <h2 className="info-center">No Results</h2>
        ) : (
